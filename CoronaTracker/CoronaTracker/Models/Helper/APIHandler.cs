...
namespace CoronaTracker.Models.Helper
{
    public class APIHandler
    {
        ...
        public async Task<CountryTimeline> LoadCountryTimelineAsync(string countryCode)
        {
            ...
            // Prepare API request objects
            string api_endpoint = "dayone/country/" + countryCode;
            var client = new RestClient(api_url_c);
             ...
             */
            //var filtered = response.Data.ToList().Where(item => item.Province.Equals("")).ToList();
            ...
            if (response.Data.Count != list.Count)
                 MessageBox.Show("check if this worked");
